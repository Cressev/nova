...
- **运行时底座**：token-meter（7.9 部分）✅；多 provider（registry 预设，PATCH 热切 base_url/model/api_key_env 三元组，设置工具条 provider-select）✅；PTY 持久终端（pty_start/write/read/list/kill，openpty+TIOCSWINSZ+滚动 256KB 缓冲，OS 沙箱内，permission=shell 走完整审批门）✅。
  ...
  - **26/09/14 五次对齐（模型列表自动获取）**：设置面板"模型"行加 ↻ 获取按钮 + datalist（可选可输）。后端 GET /api/runtime/models：OpenAI 兼容走 SDK models.list，Anthropic 走 /v1/models（含 display_name）；无 key 400、拉取失败 502 显式错误不静默。e2e 真拉 10 个 GLM 模型（glm-4.5…glm-5-turbo），下拉切换即时写入全局配置。
  - **26/09/14 六次对齐（模型列表可增删 + 候选采纳）**：对齐 dsh ModelListEditor 语义——设置面板"模型列表"区：chip 展示自定义模型（带"当前"标记 + ×删除）、"＋ 添加模型"胶囊按钮加行、"获取可用模型"拉候选→内联勾选面板（全选/取消全选 + "添加所选(N)"实时计数）→采纳写入 custom_models。后端 custom_models 持久化全局 config + PATCH 清洗去重 + payload models=当前∪custom 喂 composer 下拉。
