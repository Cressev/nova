...
项目已从产品预研进入 v1.0 MVP 代码实现阶段。当前已跑通对话式本地 Web Agent、GLM-4.7 流式输出、全局/项目级 Agent 指令注入、项目目录切换、内置指令、只读工具并行和第一版真实工具调用闭环；后续重点是补审批、真实工作树、按项目隔离会话、工具事件持久化、命令输出分片和 Langfuse 开发观测。
