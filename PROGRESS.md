...
项目已从产品预研进入 v1.0 MVP 代码实现阶段。当前已跑通对话式本地 Web Agent、GLM-4.7 流式输出、全局/项目级 Agent 指令注入、项目目录切换、内置指令、只读工具并行、第一版真实工具调用闭环、第一版 runtime event backbone、设置页 API Key 热更新，以及 ask 模式权限请求事件和前端审批卡片。后续重点切到统一 `ToolExecutor` 和 approve/deny 执行续跑，再补 `AgentSession`、上下文压缩、记忆/Skills/MCP 和 Langfuse 开发观测。

## 后续方向

- 建立统一 `ToolExecutor`，处理并行安全、独占、取消、失败、有序回填和 pending tool call。
