...
项目已从产品预研进入 v1.0 MVP 代码实现阶段。当前已跑通对话式本地 Web Agent、GLM-4.7 流式输出、全局/项目级 Agent 指令注入、项目目录切换、内置指令、第一版真实工具调用闭环、runtime event backbone、设置页 API Key 热更新、ask 模式权限请求事件和前端审批卡片。本轮已把工具执行基座推进到统一 `ToolExecutor`：18 个工具带权限/风险/hook 元数据，串行和只读并行都走同一套 hook、权限和失败兜底；设置页可以切换 permission mode、sandbox mode、approval policy，并立即影响下一次请求。后续重点切到 approve/deny 执行续跑、真实工作树、`AgentSession`、上下文压缩、记忆/Skills/MCP 和 Langfuse 开发观测。

## 后续方向

- 接入权限 approve/deny API，让审批卡片可以真正继续或拒绝执行工具。
- 补齐长命令执行模型：命令 stdout/stderr 分片、取消/中断、后台 shell output/kill。
- 基于 runtime event backbone 重构 `AgentSession`，让每个会话跨多轮保存稳定状态。
- 实现工作树创建、切换和清理。
- 增加命令输出分片和 git diff/file change 卡片。
- 完善 trace 摘要页和 Langfuse 开发调试接入。
- 下一步建议优先做权限审批 UI 和 `ToolExecutor`，因为 API Key 配好后用户会真正跑写入/shell 工具，需要可控授权和可取消执行。
