...
项目已从产品预研进入 v1.0 MVP 代码实现阶段。当前已跑通对话式本地 Web Agent、GLM-4.7 流式输出、全局/项目级 Agent 指令注入、项目目录切换、内置指令、第一版真实工具调用闭环、runtime event backbone、设置页 API Key 热更新、工具/权限/hook 基座，以及本轮新增的真实 approve/deny 续跑、shell stdout/stderr 分片、后台进程列表/output/kill、会话运行中消息排队、`/ps`/`/kill` 指令和 `.nova/memory` 长期记忆工具。前端已能真实点击审批、查看工具 hover 详情、编辑长期记忆文件，并保持服务启动在 `http://127.0.0.1:8765`。

## 后续方向

- 把本轮队列从“排队并进入上下文”推进到真正自动续跑下一轮 Agent turn。
- 把后台任务和工具审批抽成更完整的 `AgentSession` 状态机，支持页面刷新后恢复运行中 turn。
- 基于 runtime event backbone 重构 `AgentSession`，让每个会话跨多轮保存稳定状态。
- 实现工作树创建、切换和清理。
- 增加 git diff/file change 专用卡片和文件变更树。
- 完善 trace 摘要页和 Langfuse 开发调试接入。
- 下一步建议优先做 `AgentSession` 状态机和上下文压缩，因为 approve/deny、后台任务、记忆和队列已经有基础能力，下一层要解决“刷新后恢复、自动续跑、长会话不爆上下文”的稳定性。
