...
项目已从产品预研进入 v1.0 MVP 代码实现阶段。当前已跑通对话式本地 Web Agent、GLM-4.7 流式输出、全局/项目级 Agent 指令注入、项目目录切换、内置指令、只读工具并行、第一版真实工具调用闭环、第一版 runtime event backbone，以及设置页 API Key 热更新。后续重点切到基于事件骨架重构 `AgentSession`，再补统一工具执行器、权限审批、上下文压缩、记忆/Skills/MCP 和 Langfuse 开发观测。
